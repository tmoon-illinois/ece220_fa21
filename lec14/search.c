...
int main(){
    ...
    key_index = binary_search_rec(array, 0, 4, key);
    printf("key index = %d\n", key_index);
...
int binary_search_rec(int array[], int L, int H, int key){
    int M = (L+H)/2;
    if(L>H)
        return -1;
    else if(key == array[M])
        ...
        return binary_search_rec(array, M+1, H, key);
    else
        return binary_search_rec(array, L, M-1, key);
